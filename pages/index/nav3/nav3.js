...
Page({
  ...
  onLoad: function (options) {
    var that = this
    if (wx.getStorageSync("nav3Item")) {// 本地如果有 nav3Item 缓存，提前渲染
      that.setData({
        nav3Item: wx.getStorageSync("nav3Item")
      })
    }
    navUtil.getNavItem(that.data.class, that.data.num_rec_per_page, ++that.data.page, function (data) {
      wx.setStorageSync("nav3Item", data) // 覆盖缓存数据
      that.setData({
        nav3Item: data
      })
    })
  ...
  loadMore: function () {
    ...
    navUtil.getNavItem(that.data.class, that.data.num_rec_per_page, ++that.data.page, function (data) {      
      ...
      that.setData({ // 再次渲染
        nav3Item: that.data.nav3Item.concat(data),
      })
    })
    this.setData({
      hiddenLoading: !this.data.hiddenLoading
