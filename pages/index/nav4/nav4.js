...
Page({
  ...
  onLoad: function (options) {
    var that = this
    if (wx.getStorageSync("nav4Item")) {// 本地如果有 nav4Item 缓存，提前渲染
      that.setData({
        nav4Item: wx.getStorageSync("nav4Item")
      })
    }
    navUtil.getNavItem(that.data.class, that.data.num_rec_per_page, ++that.data.page, function (data) {
      wx.setStorageSync("nav4Item", data) // 覆盖缓存数据
      that.setData({
        nav4Item: data
      })
    })
  ...
  loadMore: function () {
    ...
    navUtil.getNavItem(that.data.class, that.data.num_rec_per_page, ++that.data.page, function (data) {
      ...
      that.setData({ // 再次渲染
        nav4Item: that.data.nav4Item.concat(data),
      })
    })
    this.setData({
      hiddenLoading: !this.data.hiddenLoading
